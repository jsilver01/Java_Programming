...
public class jekimFinalFrame extends JFrame {
    ...
    private void sortTableData() {
        // Sort the subjects based on the number of enrolled students
        List<Subject> sortedSubjects;
        if (sortingflag) {
            // Ascending order
            sortedSubjects = manager.sublist.stream()
                .sorted(Comparator.comparingInt(s -> s.getStudentCount()))
                .collect(Collectors.toList());
        } else {
            // Descending order
            sortedSubjects = manager.sublist.stream()
                .sorted((s1, s2) -> s2.getStudentCount() - s1.getStudentCount())
                .collect(Collectors.toList());
        }
    ...
    private void updateTableForStudent(List<Subject> subjects) {
        ...
        if (subjects != null) {
            for (Subject sub : subjects) {
                model.addRow(new Object[]{sub.getSubID(), sub.getSubName()}); 
            }
        }else {
