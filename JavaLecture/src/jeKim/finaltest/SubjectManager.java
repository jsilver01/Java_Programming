...
public class SubjectManager {
	...
	List<Subject> findStudent(String stdID) {
	    ...
	    if(subjectsWithStudent.isEmpty()) {
	    	return null;
	    }else {
	    	return subjectsWithStudent;
	    }
	}
	...
	public Subject getMaxSubject() {
	    return sublist.stream().max(Comparator.comparingInt(s -> s.stdset.size()))
	                  .orElse(null); // 수강생이 가장 많은 과목이 없는 경우 null을 반환
	}
